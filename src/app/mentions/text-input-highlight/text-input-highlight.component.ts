...
const tagIndexIdPrefix = 'limble-text-highlight-tag-id-';
...
@Component({
  selector: 'app-limble-text-input-highlight',
  templateUrl: './text-input-highlight.component.html',
  styleUrls: ['./text-input-highlight.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class TextInputHighlightComponent implements OnChanges, OnDestroy, OnInit {
  /**
  ...
  ngOnInit(): void {
    ...
    this.textInputElement.style.zIndex = '2';
  }
    ...
    [...this.tags]
      ...
      .forEach((tag) => {
        ...
        if (tagContents.length === expectedTagLength) {
          ...
          // limble-text-highlight-tag-id-${id} is used instead of a data attribute to prevent an angular sanitization warning
          parts.push(`<span class="limble-text-highlight-tag ${tagId} ${cssClass}">${escapeHtml(tagContents)}</span>`);
          prevTags.push(tag);
        }
      });
  ...
  private onMouseEnter(tag: TagMouseEvent, event: MouseEvent): void {
    tag.event = event;
    tag.target.classList.add('limble-text-highlight-tag-hovered');
    this.hoveredTag = tag;
  ...
  private onMouseLeave(tag: TagMouseEvent, event: MouseEvent): void {
    tag.event = event;
    tag.target.classList.remove('limble-text-highlight-tag-hovered');
